...
public partial class DataProcess : MonoBehaviour
{
    public Action callback;
    private int maxDownloadNum = 3;//最大下载限制
    private int maxDownCount = 5;//最大重新下载次数
    private List<DataDownload> dataDownloads = new List<DataDownload>();//下载列表
    private List<DataDownload> tmpDataDownloads = new List<DataDownload>();//暂停下载列表
    private List<DataDownload> FinishDownloads = new List<DataDownload>();//完成下载列表
    ...
    IEnumerator DwnloadRes(DataDownload dataDownload, UnityWebRequest webRequest, string fileName, string savePath, long count, Action action)
    {
        ...
        if (prevDownloadedBytes != 0)
        {
            ...
            //{
            //    File.Delete(savePath + "/" + fileName);  // 删除已有文件
            //}
    ...
    public float GetTotalSpeed()
    {
        ...
        foreach (var dataDownload in FinishDownloads)
        {
            if (dataDownload.state == DownloadState.Run)
            {
                total += dataDownload.speed;
            }
        }

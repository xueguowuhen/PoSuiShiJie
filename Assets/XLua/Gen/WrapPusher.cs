...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
            ...
            static void Init(LuaEnv luaenv, ObjectTranslator translator)
            {
                ...
                translator.RegisterPushAndGetAndUpdate<CommonNet.CMD>(translator.PushCommonNetCMD, translator.Get, translator.UpdateCommonNetCMD);
                translator.RegisterPushAndGetAndUpdate<CommonNet.Error>(translator.PushCommonNetError, translator.Get, translator.UpdateCommonNetError);
        ...
        int CommonNetCMD_TypeID = -1;
        int CommonNetCMD_EnumRef = -1;

        public void PushCommonNetCMD(RealStatePtr L, CommonNet.CMD val)
        {
            if (CommonNetCMD_TypeID == -1)
            {
                bool is_first;
                CommonNetCMD_TypeID = getTypeId(L, typeof(CommonNet.CMD), out is_first);

                if (CommonNetCMD_EnumRef == -1)
                {
                    Utils.LoadCSTable(L, typeof(CommonNet.CMD));
                    CommonNetCMD_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
                }

            }

            if (LuaAPI.xlua_tryget_cachedud(L, (int)val, CommonNetCMD_EnumRef) == 1)
            {
                return;
            }

            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, CommonNetCMD_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for CommonNet.CMD ,value=" + val);
            }

            LuaAPI.lua_getref(L, CommonNetCMD_EnumRef);
            LuaAPI.lua_pushvalue(L, -2);
            LuaAPI.xlua_rawseti(L, -2, (int)val);
            LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out CommonNet.CMD val)
        {
            LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA)
            {
                if (LuaAPI.xlua_gettypeid(L, index) != CommonNetCMD_TypeID)
                {
                    throw new Exception("invalid userdata for CommonNet.CMD");
                }
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for CommonNet.CMD");
                }
                val = (CommonNet.CMD)e;
            ...
            {
                val = (CommonNet.CMD)objectCasters.GetCaster(typeof(CommonNet.CMD))(L, index, null);
            }
        }

        public void UpdateCommonNetCMD(RealStatePtr L, int index, CommonNet.CMD val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0, (int)val))
                {
                    throw new Exception("pack fail for CommonNet.CMD ,value=" + val);
                }
            }
        ...
        int CommonNetError_TypeID = -1;
        int CommonNetError_EnumRef = -1;

        public void PushCommonNetError(RealStatePtr L, CommonNet.Error val)
        {
            if (CommonNetError_TypeID == -1)
            {
                bool is_first;
                CommonNetError_TypeID = getTypeId(L, typeof(CommonNet.Error), out is_first);

                if (CommonNetError_EnumRef == -1)
                {
                    Utils.LoadCSTable(L, typeof(CommonNet.Error));
                    CommonNetError_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
                }

            }

            if (LuaAPI.xlua_tryget_cachedud(L, (int)val, CommonNetError_EnumRef) == 1)
            {
                return;
            }

            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, CommonNetError_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for CommonNet.Error ,value=" + val);
            }

            LuaAPI.lua_getref(L, CommonNetError_EnumRef);
            LuaAPI.lua_pushvalue(L, -2);
            LuaAPI.xlua_rawseti(L, -2, (int)val);
            LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out CommonNet.Error val)
        {
            LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA)
            {
                if (LuaAPI.xlua_gettypeid(L, index) != CommonNetError_TypeID)
                {
                    throw new Exception("invalid userdata for CommonNet.Error");
                }
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for CommonNet.Error");
                }
                val = (CommonNet.Error)e;

            }
            else
            {
                val = (CommonNet.Error)objectCasters.GetCaster(typeof(CommonNet.Error))(L, index, null);
            }
        }

        public void UpdateCommonNetError(RealStatePtr L, int index, CommonNet.Error val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0, (int)val))
                {
                    throw new Exception("pack fail for CommonNet.Error ,value=" + val);
                }
            }
    ...
    public partial class StaticLuaCallbacks
    {
        internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
        {
            ...
            else if (type == typeof(CommonNet.CMD[]))
            {
                CommonNet.CMD[] array = obj as CommonNet.CMD[];
                translator.PushCommonNetCMD(L, array[index]);
                return true;
            }
            else if (type == typeof(CommonNet.Error[]))
            {
                CommonNet.Error[] array = obj as CommonNet.Error[];
                translator.PushCommonNetError(L, array[index]);
                return true;
            }
            return false;
        }
